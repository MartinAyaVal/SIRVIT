...
function resetFormulario() {
    ...
    if (comisariaSelect) {
        ...
        const conteoActual = contarUsuariosPorComisaria();
        actualizarOpcionesSelect(conteoActual);
    }
...
function resaltarVacio(input) {
    ...
    if (tieneError) {
        ...
        const mensajes = input.parentElement.querySelectorAll('.mensaje');
        mensajes.forEach(mensaje => {
...
function resaltarSelectVacio(select) {
    ...
    if (tieneError) {
        ...
        let mensaje = select.parentElement.querySelector('.mensaje');
        if (mensaje) {
            mensaje.style.display = 'block';
        }
    } else {
...
function verificarMinDocumento(input) {
    ...
    const tieneError = valor.length < 6;
...
function configurarFormularioEdicion(usuario) {
    ...
    if (comisariaSelect) {
        ...
        while (comisariaSelect.options.length > 1) {
            comisariaSelect.remove(1);
        }
        ...
        opcionesDisponibles.forEach(opcion => {
            ...
            comisariaSelect.appendChild(opt);
        });
        
        if (usuario.comisaria_rol && opcionesDisponibles.includes(usuario.comisaria_rol)) {
            comisariaSelect.value = usuario.comisaria_rol;
        } else {
            comisariaSelect.value = '';
        }
    }
    ...
    if (cargoInput) {
        ...
        cargoInput.style.cursor = 'text';
        cargoInput.style.border = '';
    }
    ...
    if (contraseñaInput) {
        ...
        if (esAccionSobreMismoUsuario(usuario.id)) {
            contraseñaInput.placeholder = 'Nueva contraseña (opcional)';
        } else {
            contraseñaInput.placeholder = 'Dejar vacío para mantener la contraseña actual';
        }
        contraseñaInput.required = false;
        contraseñaInput.readOnly = false;
        contraseñaInput.style.backgroundColor = '';
        contraseñaInput.style.color = '';
        contraseñaInput.style.cursor = 'text';
...
async function obtenerUsuarioPorId(id) {
    try {
        ...
        if (result.success && result.data) {
            console.log('Usuario encontrado en result.data:', result.data);
            return result.data;
        }
        if (result.id) {
            console.log('Usuario encontrado directamente en result:', result);
            return result;
        }
        if (result.usuario) {
            console.log('Usuario encontrado en result.usuario:', result.usuario);
            return result.usuario;
        }
        
        console.log('Formato de respuesta no reconocido, devolviendo result:', result);
        return result;
...
async function editarUsuarioHandler(id) {
    try {
        ...
        if (!usuario.comisaria_rol) {
            console.warn('El usuario no tiene campo comisaria_rol:', usuario);
        }
...
function generarContraseñaAutomatica() {
    ...
    if (!nombre) {
        nombreInput.style.border = '2px solid #ff0000';
        nombreInput.style.boxShadow = '0 0 10px rgba(255, 0, 0, 0.27)';
        
        let mensajeNombre = nombreInput.parentElement.querySelector('.mensaje');
        if (mensajeNombre) {
    ...
    if (!documento) {
        ...
        const mensajesDocumento = documentoInput.parentElement.querySelectorAll('.mensaje');
        if (mensajesDocumento.length > 0) {
            mensajesDocumento[0].style.display = 'block'; 
        }
        camposInvalidos = true;
    } else {
        ...
        const mensajesDocumento = documentoInput.parentElement.querySelectorAll('.mensaje');
        mensajesDocumento.forEach(msg => msg.style.display = 'none');
    }
    
    if (!comisaria) {
        comisariaSelect.style.border = '2px solid #ff0000';
        comisariaSelect.style.boxShadow = '0 0 10px rgba(255, 0, 0, 0.27)';
        
        let mensajeComisaria = comisariaSelect.parentElement.querySelector('.mensaje');
        if (mensajeComisaria) {
...
function contarUsuariosPorComisaria(usuarios) {
    const listaUsuarios = (usuarios && Array.isArray(usuarios)) ? usuarios : (window.usuariosActuales || []);
...
function actualizarOpcionesSelect(conteoUsuarios) {
    ...
    const limitesActuales = window.limitesConfigurados || limitesConfigurados;

    const todasLasOpciones = [
        { value: 'Comisaría Primera', text: 'Comisaría Primera' },
        { value: 'Comisaría Segunda', text: 'Comisaría Segunda' },
        { value: 'Comisaría Tercera', text: 'Comisaría Tercera' },
        { value: 'Comisaría Cuarta',  text: 'Comisaría Cuarta'  },
        { value: 'Comisaría Quinta',  text: 'Comisaría Quinta'  },
        { value: 'Comisaría Sexta',   text: 'Comisaría Sexta'   },
        { value: 'Administrador',     text: 'Administrador'     }
    ];

    const opcionesDisponibles = todasLasOpciones.filter(op => {
        const conteo = conteoUsuarios[op.value] || 0;
        const limite = limitesActuales[op.value] || 2;
    ...
    if (opcionesDisponibles.length === 0) {
        ...
        select.options[0].text = '⚠️ Todos los límites alcanzados';
        ...
        const sigueDisponible = opcionesDisponibles.some(op => op.value === valorActual);
        select.value = sigueDisponible ? valorActual : '';
    }
}
...
function actualizarContadorVisual(conteoUsuarios) {
    ...
    Object.entries(mapeoSecciones).forEach(([comisaria, tituloSeccion]) => {
        ...
        const limite = limitesActuales[comisaria] || 2;
...
function mostrarModalConfiguracionLimites() {
    if (!verificarPermisosAdministrador()) {
        mostrarError('No tienes permisos para configurar límites');
        return;
    }

    Swal.fire({
        title: 'Configurar Límites de Usuarios',
        html: `
            <div style="text-align: left;">
                <p style="margin-bottom: 15px;">Configura el máximo de usuarios permitidos por comisaría:</p>
                <div style="max-height: 400px; overflow-y: auto;">
                    ${Object.entries(limitesConfigurados).map(([comisaria, limite]) => `
                        <div style="margin-bottom: 15px; border-bottom: 1px solid #eee; padding-bottom: 10px;">
                            <label style="font-weight: bold; display: block; margin-bottom: 5px;">${comisaria}:</label>
                            <input type="number"
                                   class="limite-input"
                            ...
                            <small style="color: #666;">Mínimo: 1, Máximo: 10 usuarios</small>
                        </div>
                    `).join('')}
                </div>
            </div>
        ...
        width: '600px',
        didOpen: () => {
            window.limitesModalAbierto = true;
        },
        willClose: () => {
        ...
        preConfirm: async () => {
            ...
            inputs.forEach(input => {
                const comisaria = input.dataset.comisaria;
...
async function guardarLimitesEnServidor(nuevosLimites) {
    try {
        ...
        for (const [comisaria, limite] of Object.entries(nuevosLimites)) {
            const response = await fetch(`${GATEWAY_URL}/usuarios/admin/limites/${encodeURIComponent(comisaria)}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
            ...
            if (response.ok) {
                ...
                limitesConfigurados[comisaria] = limite;
            } else {
                resultados.push({ comisaria, success: false });
            }
        }

        await cargarUsuarios();

        mostrarExito('Límites actualizados correctamente');

    } catch (error) {
        mostrarError('Error al guardar límites: ' + error.message);
    }
}
